...
function hourForecast(forecast){
    ...
    for(let i=0;i<5;i++){
        ...
        time.innerText= (date.toLocaleDateString(undefined,'Asia/Kolkata')).replace(':00','');

        let temp= document.createElement('p');
        temp.innerText= Math.floor(forecast.list[i].main.temp_max - 273)+ ' ℃' + ' / '+ Math.floor(forecast.list[i].main.temp_min - 273)+ ' ℃';

        div.appendChild(time);
        div.appendChild(temp);

        let desc= document.createElement('p');
        desc.setAttributeNS('class','desc');
